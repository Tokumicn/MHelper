...
func BuildAccountInfo(accountArr []string) error {
	...
	accounts := make([]data.Account, len(accountArr))
...
// 1,100灵饰指南,70,0,0,0,,
func str2Account(acStr string) (data.Account, error) {
	...
	userId, err := data.ConvertStr2Int(userIdStr)
	if err != nil {
		return empty, err
	}
	...
	buyValMH, err = data.ConvertStr2Float32(buyMHStr)
	if err != nil {
		return empty, err
	}
	...
	buyValRM, err = data.ConvertStr2Float32(buyRMStr)
	if err != nil {
		return empty, err
	}
	...
	sellValMH, err = data.ConvertStr2Float32(sellMHStr)
	if err != nil {
		return empty, err
	}
	...
	sellValRM, err = data.ConvertStr2Float32(sellRMStr)
	if err != nil {
		return empty, err
	}
...
// 填充对象内容
func buildAccountFields(s data.Account) (data.Account, error) {

	// 处理购入和卖出价格信息
	bMH, bRM := utils.BuildVal(s.BuyValMH, s.BuyValRM)
	sMH, sRM := utils.BuildVal(s.SellValMH, s.SellValRM)
...
func saveAccounts(ctx context.Context, list []data.Account) error {
...
// 查询账单信息
func QueryUserAccountInfo(queryStr string, userId int64) (int64, []data.Account, error) {
	var (
		ctx      = context.Background()
		qAccount data.Account
		offset   int
		limit    = 50
	)
	...
	if userId > 0 {
		...
		// 通过关键词查询userId
		userid, ok := const_val.QueryAccountMap[queryStr]
		if ok {
			qAccount.UserId = userid
		} else {
